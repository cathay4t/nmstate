    ...
    if (!nm_conn_obj_path.is_empty()) && nm_conn_obj_path != "/" {
        ...
        let iface_name = match nm_conn.iface_name() {
            ...
            None => {
                ...
                {
                    nm_dev_from_obj_path(connection, &nm_dev_obj_path)?.name
                } else {
                    String::new()
                }
            }
...
#[cfg(feature = "query_apply")]
fn nm_ac_obj_path_nm_dev_obj_path_get(
    dbus_conn: &zbus::Connection,
    obj_path: &str,
    ...
    match proxy.get_property::<Vec<zvariant::OwnedObjectPath>>("Devices") {
